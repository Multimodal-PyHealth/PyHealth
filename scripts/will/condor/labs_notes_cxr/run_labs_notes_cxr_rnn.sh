#!/usr/bin/env bash
# HTCondor executable — labs+notes+CXR RNN mortality run.
#
# Condor equivalent of
# scripts/will/lambda-labs/labs_notes_cxr/tmux_run_labs_notes_cxr_rnn_variant.py:
# same task (notes_labs_cxr), same model (rnn), same hyperparameter defaults. GPU
# selection (nvidia-smi / CUDA_VISIBLE_DEVICES) is dropped since Condor
# assigns the GPU via request_gpus / cgroups.
#
# usage: run_labs_notes_cxr_rnn.sh <seed>
# to remove logs: rm -rf logs/condor/*
# to remove cache: rm -rf /shared/rsaas/wp14/pyhealth_cache_labs_notes_cxr/*
set -euo pipefail

SEED="${1:?usage: run_labs_notes_cxr_rnn.sh <seed>}"
...
CXR_VARIANT="${CXR_VARIANT:-default}"
CACHE_DIR="${CACHE_DIR:-/shared/rsaas/wp14/pyhealth_cache_labs_notes_cxr}"
...
WANDB_PROJECT="${WANDB_PROJECT:-pyhealth-multimodal-labs-notes-cxr}"
...
JOB_TAG="rnn_labs_notes_cxr_s${SEED}_c${_CONDOR_CLUSTER_ID:-local}_p${_CONDOR_PROCNO:-0}"

echo "========================================================"
echo "  Labs+notes+CXR RNN run  |  ${JOB_TAG}"
echo "  $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
echo "  Conda env  : ${CONDA_ENV}"
echo "  EHR root   : ${EHR_ROOT}"
echo "  Note root  : ${NOTE_ROOT}"
echo "  CXR root   : ${CXR_ROOT} (variant=${CXR_VARIANT})"
