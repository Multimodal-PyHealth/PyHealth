...
cxr_root = "/home/ubuntu/mimiciv-data/CXR-jpg"
cxr_variant = "default"
cache_dir = "/home/ubuntu/mimiciv-data/pyhealth_cache_labs_notes_cxr"
...
use_old_cache = False
use_wandb = True
wandb_project = f"pyhealth-multimodal-labs-notes-cxr-seed-{seed}"
wandb_run_name = None  # defaults to "{model}_seed{seed}" if unset
cuda_visible_devices = "0"
session_name = f"jambaehr_labs_notes_cxr_s{seed}"
...
log_dir = logs_dir
log_tag = f"jambaehr_labs_notes_cxr_s{seed}"

flags = [
    f"--ehr-root {ehr_root}",
    f"--note-root {note_root}",
    f"--cxr-root {cxr_root}",
    f"--cxr-variant {cxr_variant}",
    f"--cache-dir {cache_dir}",
    f"--task notes_labs_cxr{' --dev' if dev else ''}",
    "--model jambaehr",
    f"--embedding-dim {embedding_dim}",
    f"--hidden-dim {hidden_dim}",
    f"--heads {heads}",
    f"--num-layers {num_layers}",
    f"--jamba-transformer-layers {jamba_transformer_layers}",
    f"--jamba-mamba-layers {jamba_mamba_layers}",
    f"--mamba-state-size {mamba_state_size}",
    f"--mamba-conv-kernel {mamba_conv_kernel}",
    f"--dropout {dropout}",
]
if use_amp:
...
# ── Step 4: Confirm which note sections + CXR variant were used ─────────────
print(f"""
### STEP 4: Confirm which discharge-note AND radiology-note sections
### NotesLabsCXRMIMIC4 filtered to (e.g. discharge: "chief complaint";
### radiology: "indication", "impression"), and which CXR variant was loaded.
### Both logged once at INFO level by pyhealth.tasks.multimodal_mimic4 /
### pyhealth.datasets.MIMIC4Dataset when the task/dataset are constructed.

grep "filtering discharge notes to sections" {log_dir}/{log_tag}.out
grep -i "cxr" {log_dir}/{log_tag}.out | head -20
""")
