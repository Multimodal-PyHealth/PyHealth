...
The runner on this branch does accept `--observation-window-hours`; the paper
cells leave it unset, which means full stay.

## Flags a backbone ignores

The runner logs these at startup, so a config can never appear to set something
...
**Labs + Notes only**, and they predate the optimizer fix in this branch: the
runner hardcoded `max_grad_norm=0.5` and Adam `eps=1e-6` for
`bottleneck_transformer` alone, so that cell was not compute-matched to the
other five. Those six cells need re-running before they go in a table.

Two things this branch's runner still lacks, which the published numbers were
produced with: train-split lab standardization (`fit_lab_standardizer`) and
`write_run_config`, so runs started from here get no `run_config.json` and no
lab z-scoring. Cells run from this branch will not reproduce the numbers in
`rian/scripts/README.md` until those are ported.
