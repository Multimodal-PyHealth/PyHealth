#!/usr/bin/env bash
# Paper cell: Labs + Notes + CXR x Transformer, seed ${SEED:-1}.
...
launch "paper_labs_notes_cxr_transformer_seed${SEED}_${IMAGE_BACKBONE}" \
  --task notes_labs_cxr --model transformer \
  --ehr-root "$EHR_ROOT" \
  --note-root "$NOTE_ROOT" \
  --cxr-root "$CXR_ROOT" \
  --cxr-variant sunlab \
