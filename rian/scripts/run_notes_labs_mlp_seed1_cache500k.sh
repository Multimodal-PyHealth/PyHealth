#!/usr/bin/env bash
# Paper cell: notes_labs x MLP x seed 1 — 500k [CLS] cache (cap sweep point between 200k and 1M).
# Same tree and flags as run_notes_labs_mlp_seed1.sh; only --max-frozen-text-cache differs.
set -euo pipefail
...
NAME=notes_labs_mlp_seed1_cache500k
