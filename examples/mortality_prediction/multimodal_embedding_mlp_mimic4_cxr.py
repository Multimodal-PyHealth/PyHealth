"""Unified multimodal embedding + MLP runner for MIMIC-IV + CXR.
...
Quick start:
    ...
    multimodal_embedding_mlp_mimic4_cxr.py \
      --quick-test
...
from pyhealth.models import MLP, UnifiedMultimodalEmbeddingModel
...
def _build_run_output_path(args: argparse.Namespace) -> str:
    ...
    return os.path.join(
        os.getcwd(), "output", "multimodal_embedding_mlp_mimic4_cxr", run_tag
    )
...
def run(args: argparse.Namespace) -> Tuple[int, int]:
    ...
    print(f"MLP unified mode: {model._use_unified}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run unified multimodal embedding + MLP on "
