...
Quick start:
    ...
    multimodal_embedding_jamba_mimic4_cxr.py \
      --quick-test
...
def _build_run_output_path(args: argparse.Namespace) -> str:
    ...
    run_tag = (
        ...
        f"_attn{args.num_transformer_layers}"
        f"_mamba{args.num_mamba_layers}"
        f"_heads{args.heads}"
    ...
    return os.path.join(
        os.getcwd(), "output", "multimodal_embedding_jamba_mimic4_cxr", run_tag
    )
...
def run(args: argparse.Namespace) -> Tuple[int, int]:
    ...
    print(f"JambaEHR unified mode: {model._use_unified}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run unified multimodal embedding + JambaEHR on "
