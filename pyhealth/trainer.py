...
def resolve_amp_dtype(amp_dtype: str, use_amp: bool = False) -> torch.dtype:
    ...
    so every other value selected fp16 without a message. A typo, or the spelling
    ``"bfloat16"``, silently changed the precision of a run. fp16 also needs a
    GradScaler, so the silent path changed the gradient behaviour as well.
    """
...
class Trainer:
    ...
    def train(
        ...
        _amp_dtype = resolve_amp_dtype(amp_dtype, use_amp)
