...
def write_run_config(exp_path, config):
    ...
    that produced it, so a frozen-encoder run and a fine-tuned one are
    indistinguishable once the job's stdout is gone. Record the resolved
    settings, not the raw flags, so derived conditions are recoverable.
    """
    record = {
